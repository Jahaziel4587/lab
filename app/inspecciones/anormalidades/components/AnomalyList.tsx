  ...
  if (loading) {
    return (
      <div className="flex min-h-36 items-center justify-center rounded-2xl border border-white/10 bg-black/15">
        ...
        <span className="ml-3 text-sm text-white/55">Cargando anormalidades...</span>
      </div>
    );
  }
    ...
    return (
      <div className="rounded-2xl border border-red-400/20 bg-red-400/[0.06] p-5">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="font-medium text-red-100">No fue posible cargar las anormalidades</p>
            <p className="mt-1 text-sm text-red-100/65">{error}</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div>
            ...
            <div className="flex min-w-0 items-start gap-3">
              ...
              <div className="min-w-0">
                <p className="font-medium text-white">
                  {anomaly.title.trim() || `Reporte pendiente de título #${anomalies.length - index}`}
                </p>
                <p className="mt-1 text-xs text-white/40">
                  Responsable: {anomaly.responsiblePmName || anomaly.responsiblePmEmail || "Sin asignar"}
                </p>
                <span className={`mt-2 inline-flex rounded-full border px-2.5 py-1 text-xs ${decisionClass(anomaly)}`}>
                  {decisionLabel(anomaly)}
                </span>
              </div>
            </div>
            <ChevronRight size={19} className="shrink-0 text-white/35 transition group-hover:translate-x-0.5 group-hover:text-emerald-300" />
          </button>
        ))}
      </div>
      ...
      <button
        type="button"
        onClick={onReportNew}
        className="mt-5 flex min-h-14 w-full items-center justify-center gap-2 rounded-2xl border border-dashed border-emerald-400/30 bg-emerald-400/[0.06] px-5 py-3 font-medium text-emerald-200 transition hover:bg-emerald-400/10"
      >
        <Plus size={18} />
